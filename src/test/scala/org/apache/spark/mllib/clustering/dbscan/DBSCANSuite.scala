...
class DBSCANSuite extends LocalDBSCANNaiveSuite with MLlibTestSparkContext with Matchers {

  private val dataFile = "labeled_data.csv"

  private val corresponding = Map(3 -> 1D, 2 -> 2D, 1 -> 3D, 0 -> 0D)

  test("dbscan") {

    val data = spark.sparkContext.textFile(getFile(dataFile).toString())

    val parsedData = data.map { s =>
      val arr = s.split(',')
      val vector = Vectors.dense(arr.slice(0, 2).map(_.toDouble))
      DBSCANPoint(vector = vector, arr(2).toLong)
    }
